...
func TestTmuxRuntime_RemainOnExit(t *testing.T) {
	...
	// Session should still exist (we can't check sessionExists from outside package)
	// Just verify the process is still in a valid state
	assert.NotEqual(t, runtime.StateFailed, p.State())
