...
func TestRemoveSession(t *testing.T) {
	...
	t.Run("auto-removes workspace when session is removed", func(t *testing.T) {
		...
		wsAuto, err := wsManager.Create(context.Background(), workspace.CreateOptions{
		...
		_, err = wsManager.ResolveWorkspace(context.Background(), workspace.Identifier(wsAuto.ID))
		require.NoError(t, err)
	...
	t.Run("keeps workspace when --keep-workspace flag is used", func(t *testing.T) {
		// Create a new workspace marked as auto-created with unique name
		ws2, err := wsManager.Create(context.Background(), workspace.CreateOptions{
		...
		_, err = wsManager.ResolveWorkspace(context.Background(), workspace.Identifier(ws2.ID))
		assert.NoError(t, err)
	})
	...
	t.Run("does not remove workspace when used by other sessions", func(t *testing.T) {
		...
		ws3, err := wsManager.Create(context.Background(), workspace.CreateOptions{
			Name:        wsName,
			AutoCreated: true,
		})
		require.NoError(t, err)
		...
		_, err = wsManager.ResolveWorkspace(context.Background(), workspace.Identifier(ws3.ID))
	...
	t.Run("does not remove manually created workspace", func(t *testing.T) {
		...
		ws4, err := wsManager.Create(context.Background(), workspace.CreateOptions{
		...
		// Verify workspace still exists
		_, err = wsManager.ResolveWorkspace(context.Background(), workspace.Identifier(ws4.ID))
		assert.NoError(t, err)
	})
}
