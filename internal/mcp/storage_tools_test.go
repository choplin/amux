...
func TestSeparatedStorageTools(t *testing.T) {
	...
	t.Run("deprecated tools still work", func(t *testing.T) {
		t.Run("storage_write with workspace", func(t *testing.T) {
			request := mcp.CallToolRequest{
				Params: mcp.CallToolParams{
					Name: "storage_write",
					Arguments: map[string]interface{}{
						"workspace_identifier": ws.ID,
						"path":                 "deprecated.txt",
						"content":              "Deprecated but working",
					},
				},
			}

			result, err := server.handleStorageWrite(ctx, request)
			require.NoError(t, err)
			assert.NotNil(t, result)

			// Verify file was created
			content, err := os.ReadFile(filepath.Join(ws.StoragePath, "deprecated.txt"))
			require.NoError(t, err)
			assert.Equal(t, "Deprecated but working", string(content))
		})

		t.Run("storage_write fails with both identifiers", func(t *testing.T) {
			request := mcp.CallToolRequest{
				Params: mcp.CallToolParams{
					Name: "storage_write",
					Arguments: map[string]interface{}{
			...
			_, err := server.handleStorageWrite(ctx, request)
			assert.Error(t, err)
			assert.Contains(t, err.Error(), "only one of")
		})

		t.Run("storage_write fails with neither identifier", func(t *testing.T) {
			request := mcp.CallToolRequest{
				Params: mcp.CallToolParams{
					Name: "storage_write",
					Arguments: map[string]interface{}{
			...
			_, err := server.handleStorageWrite(ctx, request)
			assert.Error(t, err)
			assert.Contains(t, err.Error(), "either workspace_identifier or session_identifier")
		})
	})
}
